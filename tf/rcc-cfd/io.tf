...
variable "pvserver_ports" {
  description = "Allowable ports for reverse SSH connection to paraview server"
  type        = list(string)
  default     = ["11000"]
}

variable "pvserver_source_ip_range" {
  description = "Allowable IP range of instances that can connect to paraview server"
  type        = list(string)
  default     = ["0.0.0.0/0"]
}
